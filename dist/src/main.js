...
app.post('/api/replicache/:op', async (req, res, next) => {
    await handleRequest(req, res, next, mutators);
});
app.get('/api/replicache/poke', async (req, res) => {
    await handlePoke(req, res);
});
app.get('/api/replicache/createSpace', async (_req, res, next) => {
    await handleReadRequest(res, next);
});
app.post("/sync-issues", async (req, res, _next) => {
